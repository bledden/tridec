...
> `BLOCK≥256` refusal (the "honest negative" below) is **resolved**: triton-msl's
...
> Requires triton-msl `≥ b82136b`; on older triton-msl relay@256 loudly refuses.
...
everything at `BLOCK=32` as a workaround for two triton-msl codegen gaps).
...
workaround + the triton-msl limitation repros.

## Why the pin could be lifted

`megakernel_metal_spike.md` documented two triton-msl gaps that forced
`BLOCK=32` on Metal:
...
triton-msl **CODEGEN_VERSION 2026.06.13** (commit `0a1eafb`) fixes Bug 1
(barriers honored) and fixes Bug 2 **for the BP-megakernel shape** via its
register-array spine. Caches (`~/.triton/cache`, `~/.cache/triton_msl`) were
cleared before this run, per the codegen bump.

## Environment

- Apple M4 Max, macOS (Darwin 24.6.0), `/tmp/pq-metal-venv`, python 3.14.4
- triton-msl `0.1.0a1 @ 0a1eafb` (CODEGEN_VERSION 2026.06.13), triton 3.7.0,
...
fix landed for *that* shape. This is reported back to the triton-msl thread
...
reduction unsupported). triton-msl's B/C/A branch (`@ b82136b`, CODEGEN
...
- At `BLOCK=256` with the default `num_warps=4`, `num_threads = 4×32 = 128`, so
  `n = BLOCK/num_threads = 2` elements/thread. triton-msl's **base path emits
...
Verified on M4 Max, 2000-shot BB cell, vs the triton-msl B/C/A branch
...
**Requires** triton-msl `≥ b82136b`; older → relay@256 loudly refuses (safe).
